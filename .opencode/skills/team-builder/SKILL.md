...
1. **`claude agents` command** (primary) — run `claude agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `architect`), and built-in agents. This automatically covers ECC marketplace installs without any path configuration.
...
- **Plugin agents** are prefixed with `plugin-name:` (e.g., `security-reviewer`). Use the part after `:` as the agent name and the plugin name as the domain.
